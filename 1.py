...
class TRPCA:
    ...
    def T_product(self,A,B):
        ...
        return C
    def ADMM(self,left,right,X,Y,lamb,beita,gama):
        l, m, n = X.shape
        c = Y.shape[0]
        r = 30 #reduce
        rho = 1.1
...
if __name__ =='__main__':
    ...
    # view1 = spy.imshow(classes=label, title="gt",cmap='viridis')
    # plt.savefig('/data/LOH/TSPLL_label/Classification maps/Ablation Analysis/longkou/lambda=1,1,0 no_graph/gt.jpg')
    # ax = plt.gca()
    # # 隐藏坐标轴
    # ax.axis('off')
    # plt.savefig(
    #     '/data/LOH/TSPLL_label/Classification maps/Ablation Analysis/longkou/lambda=1,1,0 no_graph/gt.jpg',
    ...
    for i in range(x_train.shape[1]):
        x = x_train[:, i, :].reshape(b, 1, PATCH_SIZE*PATCH_SIZE)
    ...
    for lamb in lamb_set:
        for beita in beita_set:
            for gama in gama_set:
                M, L, E, P, W, G, Q= ours.ADMM(left,right,x_train,train_label,lamb,beita,gama)
                ...
                view2 = spy.imshow(classes=tup, title="acc-gt-97.74", cmap='viridis')
                ax = plt.gca()
                # 隐藏坐标轴
                ax.axis('off')
                plt.savefig('/data/LOH/TSPLL_label/ours-longkou.jpg',dpi=300, bbox_inches='tight')
                print(acc)
                ...
                view4 = spy.imshow(classes=tupQ, title="lk-91")
                plt.savefig('91.98 lk.jpg')
                ...
                # print acc
                # with open('/data/LOH/TSPLL_label/Classification maps/Sensitivity Analysis/longkou/regularization parameters/result1.txt', 'a', encoding='utf-8') as file:
                #     sys.stdout = file
                #     print(lamb,beita,gama)
    ...
    for lamb in lamb_set:
        for beita in beita_set:
            for gama in gama_set:
                ...
                acc,tup = nn_unique(X_train_reduced, train_label_list, X_test_reduced, test_label_list, random_idx, label)
                view2 = spy.imshow(classes=tup, title="acc-gt-98.46")
                ax = plt.gca()
                ax.axis('off')
                plt.savefig('/data/LOH/TSPLL_label/ours-salinas.jpg', dpi=300, bbox_inches='tight')
                ...
                # print acc
                # with open('/data/LOH/TSPLL_label/Classification maps/Sensitivity Analysis/Salinas/regularization parameters/result1.txt', 'a', encoding='utf-8') as file:
                #     sys.stdout = file
                #     print(lamb,beita,gama)
                #     print(f"P*X Total Precision: {acc['total_precision']* 100:.4f}")
                #     print(f"P*X average_precision: {acc['average_precision'] * 100:.4f}")
    ...
    for lamb in lamb_set:
        for beita in beita_set:
            for gama in gama_set:
                ...
                acc, tup = nn_unique(X_train_reduced, train_label_list, X_test_reduced, test_label_list, random_idx, label)
                view2 = spy.imshow(classes=tup, title="acc-gt-97.50")
                ax = plt.gca()
                # 隐藏坐标轴
                ax.axis('off')
                plt.savefig('/data/LOH/TSPLL_label/ours-indian.jpg', dpi=300, bbox_inches='tight')
                ...
                # print acc
                # with open(
                #         '/data/LOH/TSPLL_label/Classification maps/Sensitivity Analysis/indian_pines/regularization parameters/result.txt',
